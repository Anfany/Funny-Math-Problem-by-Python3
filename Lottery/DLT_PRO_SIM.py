...
if __name__ == "__main__":
    ...
    print('DLT：计算概率阶段')
    # 模型
    mm = yanzheng.MODEL(ff, list(range(1, 36)), 5)
    ...
    for ss in range(1, maxcount + 1):
        ...
        plt.legend(['本文方法(统计：%d条)' % int(comb(len(mm.red), mm.redcount)), '理论概率'])
        plt.title('DLT:选择%s个球的命中情况概率对比' % ss)
        plt.xlim(- 0.5, len(col_labels) - 0.5)
        plt.xticks([])
        plt.grid()
        plt.savefig(r'C:\Users\GWT9\Desktop\%s_dlt_test.jpg' % ss)
